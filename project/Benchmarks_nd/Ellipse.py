...
# %%
base_params = (10_000, 1, 1,
               0, 0)


class Ellipse(Benchmark):
    ...
    def __init__(self, params: Annotated[Tuple[number], 5] = base_params,
        ...
        super(Ellipse, self).__init__(params, lower, upper, D)
    ...
    def _get_value(self, inp: npt.NDArray) -> npt.NDArray:
        ...
        s_1 = np.sum(p[0] ** ((np.arange(self.D) + 1 - p[1]) / (self.D-p[2])) *
                     (inp + p[3])**2, axis=1) + p[4]
